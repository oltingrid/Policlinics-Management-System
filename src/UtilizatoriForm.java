...
public class UtilizatoriForm {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					UtilizatoriForm window = new UtilizatoriForm(6,1);
	 ...
	 */
	public UtilizatoriForm(long userId,int accessCode) {
	 ...
	 public void table_load()
	 {
		 ...
		 try {
				...
				if(this.loginUserAccessCode==1) {
					...
					sqlText = "SELECT * FROM utilizatori WHERE id_utilizator="+id.toString(); 
				} else {
	 ...
	 private void cmdSaveUpdate(){
			try {
				...
				String UPDATE = "UPDATE  utilizatori SET CNP=?,nume=?,prenume=?,adresa=?,telefon=?,email=?,cont_iban=?,numar_contract=?,"
						+ "parola=?,id_tip_utilizator=?  WHERE id_utilizator=?";
	 ...
	 private void cmdSaveNew(){
			try {
				...
				String INSERT = "INSERT INTO utilizatori (CNP,nume,prenume,adresa,telefon,email,cont_iban,numar_contract,"
						+ "parola,id_tip_utilizator) VALUES (?, ?,?,?,?,?,?,?,?,?)";
