...
public class RapoarteAnaliza {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					RapoarteAnaliza window = new RapoarteAnaliza(8,5);
	 ...
	 */
	public RapoarteAnaliza(long userId,int accessCode) {
	 ...
	 public void table_load()
	 {
		 ...
		 try {
				...
				if(this.loginUserAccessCode==1) {
					...
					sqlText = "SELECT * FROM pacienti WHERE id_utilizator="+id.toString(); 
				} else {
	...
	private void cmdloadSelectdRecord(String selectedID) {
		...
		IstoricAnalizeRaportForm frame = new IstoricAnalizeRaportForm(id_pacient);
